...
Contributions by: Fabio Civillini, Matteo Martinelli, Francesco Mambretti, Davide E. Galli, Tommaso Rovelli
...
- Include moves that change the box geometry 
...
// Give each rank a different set of parameters for parallel random numbers generation
	...
	int n_update_verlet=3;    //set this number as the highest that doesn't change the output
	...
	for(iblk=1; iblk <= nblkt; ++iblk)
  	{
    	...
    	{
		//Update Verlet list
			if(istep%n_update_verlet==0) Update_neighbors(); 			
...
//Simulation  	
  	for(iblk=1+oblk; iblk <= nblk; ++iblk) //loop over blocks
  	{
			...
			//Update Verlet list
			if(istep%n_update_verlet==0) Update_neighbors();		//Maybe here is useless but maybe not
...
void Input(void)
{
	...
	{
	 	ifstream ReadSeed, file_input, file_input_distrib;
	  
	//Read input informations
		file_input.open("input.dat");
		file_input_distrib.open("input_distrib.dat");
	...
	// Read moves parameters
		...
		file_input >> deltaAngle; // delta tan of the box angle 
		file_input >> deltaAspectRatio; // delta of boxx for aspect ratio move
	...
	// Read distribution parameters
		...
		file_input_distrib >> nbins_angle;
		file_input_distrib >> nbins_ld_cm_x;
		file_input_distrib >> nbins_ld_cm_o1;
		file_input_distrib >> nbins_ld_cm_o2;
		file_input_distrib >> nbins_ene;
		file_input_distrib >> nbins_box;

		file_input_distrib >> bin_size_angle;
		file_input_distrib >> bin_size_ld_cm_x;
		file_input_distrib >> bin_size_ld_cm_o1;
		file_input_distrib >> bin_size_ld_cm_o2;
		file_input_distrib >> bin_size_ene;
		file_input_distrib >> bin_size_boxx;
		file_input_distrib >> bin_size_boxy;

		file_input_distrib >> left_value_angle;
		file_input_distrib >> left_value_ld_x;
		file_input_distrib >> left_value_ld_o1;
		file_input_distrib >> left_value_ld_o2;
		file_input_distrib >> left_value_ene;
		file_input_distrib >> left_value_boxx;
		file_input_distrib >> left_value_boxy;
	...
	// Print initial conditions
		...
		cout << "Variation of the tan of the box angle= " << deltaAngle << endl;
		cout << "Variation of the Lx side, for the Aspect Ratio changing= " << deltaAspectRatio << endl;
...
// Broadcast input params to all ranks
	...
	MPI_Bcast(&deltaAngle,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);
	MPI_Bcast(&deltaAspectRatio,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);

	MPI_Bcast(&nbins_angle,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);
	MPI_Bcast(&nbins_ld_cm_x,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);
	MPI_Bcast(&nbins_ld_cm_o1,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);
	MPI_Bcast(&nbins_ld_cm_o2,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);
	MPI_Bcast(&nbins_ene,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);
	MPI_Bcast(&nbins_box,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);

	MPI_Bcast(&bin_size_angle,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);
	MPI_Bcast(&bin_size_ld_cm_x,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);
	MPI_Bcast(&bin_size_ld_cm_o1,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);
	MPI_Bcast(&bin_size_ld_cm_o2,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);
	MPI_Bcast(&bin_size_ene,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);
	MPI_Bcast(&bin_size_boxx,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);
	MPI_Bcast(&bin_size_boxy,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);

	MPI_Bcast(&left_value_angle,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);
	MPI_Bcast(&left_value_ld_x,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);
	MPI_Bcast(&left_value_ld_o1,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);
	MPI_Bcast(&left_value_ld_o2,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);
	MPI_Bcast(&left_value_ene,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);
	MPI_Bcast(&left_value_boxx,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);
	MPI_Bcast(&left_value_boxy,1,MPI_DOUBLE,rank_0,MPI_COMM_WORLD);
	...
	igang = isofq_clo + nbinqo;
	ildcm_x = igang + nbins_angle;
	ildcm_o1 = ildcm_x + nbins_ld_cm_x;
	ildcm_o2 = ildcm_o1 + nbins_ld_cm_o1;
	iene= ildcm_o2 + nbins_ld_cm_o2;
	iboxx = iene + nbins_ene;
	iboxy = iboxx + nbins_box;
	n_props = iboxy + nbins_box;
	...
	ene_dist=new long double[nbins_ene]();
	angle_dist=new long double[nbins_angle]();
	boxx_dist=new long double[nbins_box]();
	boxy_dist=new long double[nbins_box]();

	for(int i=0; i < nbins_ene; ++i) ene_dist[i]=0;
	for(int i=0; i < nbins_angle; ++i) angle_dist[i]=0;
	for(int i=0; i < nbins_box; ++i) ene_dist[i]=0;
	for(int i=0; i < nbins_box; ++i) ene_dist[i]=0;
	...
	{	
		...
		if (my_rank==rank_0) 
		{
			...
			cout << "Read initial configuration from file output.config.final.txt " << endl << endl;
		  	ReadConf.open("OutputConfig/output.config.final0.txt");
...
void Move(int acc_index) // single particle translation
{
	...
	//New
		delta_x = delta[acc_index]*(Rannyu() - 0.5);
		delta_y = delta[acc_index]*(Rannyu() - 0.5);
		xnew = particle[o].x + delta_x;
		ynew = particle[o].y + delta_y;
	...
	//Metropolis test
		...
		if(p >= Rannyu())  
		{
			/*if(acc_index==0) {
				...
				int po = particle[o].pointer;
				double dl = Distance(particle[o], particle[po]);

				ofstream output("dimer_break_"+to_string(my_rank)+".out",ios::app);

				if(dl > 0.4)  {
					
					output << "Probably a dimer break just happened" << endl;
					output << "Dimer distance: " << dl << endl << endl;

				}

				output.close();

				Boltzmann_Print(xnew, ynew, particle[o].x, particle[o].y, o);
...
void Move_Angle()
{
	...
	double energy_old=0;
	double energy_new=0;
	Particle *old_particle = new Particle[npart]();
	double xm, ym, dr, cosPhi, senPhi;
	...
	for(int i=0; i<npart-1; ++i) {
		for(int j=i+1; j<npart;++j) {
			double d = Distance(particle[i],particle[j]);
			energy_old += V_Pot(d);
		}
	}

	//store old particle position
	for(int i=0; i<npart; ++i) old_particle[i] = particle[i];

	//modify tan(a)
	new_tga = tga + deltaAngle*(Rannyu()-0.5);
	tga = new_tga;

	inv_new_tga = 1.0/new_tga;
	inv_old_tga = 1.0/old_tga;

	//Move the box and adjust the mass center and the molecules position
	for(int i=0; i<nspin; ++i) {

		for(int j=0; j<npart; j++) {   //select the dimers
		...
		double disp = ym * (inv_new_tga-inv_old_tga);
		xm = Pbcx(xm + disp, ym);

		particle[o].xm = xm;
		particle[po].xm = xm;

		particle[o].x = Pbcx(xm+dr*cosPhi, ym+dr*senPhi); 	particle[po].x = Pbcx(xm-dr*cosPhi, ym-dr*senPhi);	
		particle[o].y = Pbcy(ym+dr*senPhi);					particle[po].y = Pbcy(ym-dr*senPhi);
	...
	double p = exp(beta*(energy_old-energy_new));
	if(p > Rannyu()) {
		accepted[ia]++;

	}else{
		//restore old quantities
		for(int i=0; i<npart; ++i) particle[i] = old_particle[i];
		tga=old_tga;
	}
...
void Move_AspectRatio() 
{
	int o, po;
	double old_boxx,old_boxy;
	double energy_old=0;
	double energy_new=0;
	Particle *old_particle = new Particle[npart]();
	double xm, ym, dr, cosPhi, senPhi;
	...
	//modify box lenght
	boxx += deltaAspectRatio * (Rannyu()-0.5);
	boxy = A/boxx;
	...
	for(int i=0; i<nspin; ++i) {
		...
		double dilat = boxx/old_boxx;
		xm = Pbcx(xm*dilat, ym/dilat);
		ym = Pbcy(ym/dilat);
...
void Update_neighbors() // Update neighbors lists
{
	double dR;
    int i,j,k=0;
...
void Measure()
{
	...
	double dr, dx, dy, dr_cm;
	double phi, cos2phi;
	bool last_row, first_col;
...
//reset linear distribution cm 
	for( k=ildcm_x; k<nbins_ld_cm_x;++k) walker[k]=0;
	for( k=ildcm_o1; k<nbins_ld_cm_o1;++k) walker[k]=0;
	for( k=ildcm_o2; k<nbins_ld_cm_o2;++k) walker[k]=0;

//reset energy distribution
	for(k=iene;k<nbins_ene;++k) walker[k]=0;
    
//cycle over pairs of particles
	for ( i=0; i<npart-1; ++i)
	{
		if(particle[i].i_cluster % nx == 0) first_col = true;
		else first_col = false;

		if(particle[i].i_cluster % ny == (ny-1)) last_row = true;
		else last_row = false;
		...
		for ( j=i+1; j<npart; ++j)
		{
			...
			//update the linear distributions
			if(particle[i].i_cluster != particle[j].i_cluster){
				dr_cm = sqrt( pow(particle[i].xm-particle[j].xm,2 ) + pow(particle[i].ym-particle[j].ym,2 ));
				if (int(particle[i].i_cluster/nx) == int(particle[j].i_cluster/nx)){ //if clusters are in the same horizontal row
					bin = ildcm_x + (int)((dr_cm-left_value_ld_x)/bin_size_ld_cm_x);
					if(bin < ildcm_x + nbins_ld_cm_x) ++walker[bin];
				}
				else if ( (abs(particle[i].i_cluster - particle[j].i_cluster)) % nx == 0 ){ //if clusters are in the first diagonal line (/)
					bin = ildcm_o1 + (int)((dr_cm-left_value_ld_o1)/bin_size_ld_cm_o1);
					if(bin < ildcm_o1 + nbins_ld_cm_o1) ++walker[bin];
				}
				else if ( (abs(particle[i].i_cluster - particle[j].i_cluster)) % (nx-1) == 0 ){ //if clusters are in the second diagonal line (\)
					bin = ildcm_o2 + (int)((dr_cm-left_value_ld_o2)/bin_size_ld_cm_o2);
					if(bin < ildcm_o2 + nbins_ld_cm_o2) ++walker[bin];
				}
				else if ( first_col && ( particle[j].i_cluster == (particle[i].i_cluster + 2*nx -1)) ){  //if the i
					bin = ildcm_o2 + (int)((dr_cm-left_value_ld_o2)/bin_size_ld_cm_o2);
					if(bin < ildcm_o2 + nbins_ld_cm_o2) ++walker[bin];
				}
				else if ( last_row && ( particle[i].i_cluster == ( particle[j].i_cluster + nx*(ny-1)+1 ))) {
					bin = ildcm_o2 + (int)((dr_cm-left_value_ld_o2)/bin_size_ld_cm_o2);
					if(bin < ildcm_o2 + nbins_ld_cm_o2) ++walker[bin];
				}
	...
	for( i=0; i<npart; ++i) {
		...
		phi = particle[i].phi;
		cos2phi += cos(phi)*cos(phi);
	}

	cos2phi /= (double)(npart);
...
//Compute S(q) of dimers along x axis
   ...
   for ( i=0; i<nbinqy; ++i)
   {
	...
	//angle distribution
	for(int i = igang; i<igang+nbins_angle;++i) walker[i]=0;
	...
	//boxx distribution
	for(int i = iboxx; i<iboxx+nbins_box;++i) walker[i]=0;
	...
	//boxy distribution
	for(int i = iboxy; i<iboxy+nbins_box;++i) walker[i]=0;
...
{   
	long double err, r, gdir, gdis, a, gdia, ldcm_x, ldcm_o1, ldcm_o2, ene_dist, boxx_dist, boxy_dist;
	long double stima;
	long double ene_2, ene;
	long double op;
    int kk, k;
 
	ofstream Gave, Gspin_x, Gspin_obl, Epot, Sofq_x, Sofq_y, Sofq_o, Sofq_clx, Sofq_cly, Sofq_clo, CS, Gangle, OrderPar, LinDistX, LinDistO1, LinDistO2, EneDist, BoxxDist, BoxyDist; //output streams
	...
	cout << "Acceptance rate of box angle move " << double(mpi_accepted[ia])/double(mpi_attempted[ia]) << endl;
	cout << "Acceptance rate of Aspect Ratio move " << double(mpi_accepted[ias])/double(mpi_attempted[ias]) << endl;
    
	Epot.open("output.epot.txt", ios::app);
	CS.open("output.specific_heat.txt", ios::app);
	OrderPar.open("output.order_parameter.txt", ios::app);
	if (iblk == nblk)
	{
		...
		Gangle.open("output.gangle.txt");
		LinDistX.open("output.ldcmx.txt");
		LinDistO1.open("output.ldcmo1.txt");
		LinDistO2.open("output.ldcmo2.txt");
		EneDist.open("output.energy.distribution.txt");
		BoxxDist.open("output.boxx.distribution.txt");
		BoxyDist.open("output.boxy.distribution.txt");
	}
	...
	//Order Parameter
	stima = mpi_blk_av[iv4]/mpi_blk_norm;
	glob_av[iv4] += stima;
	glob_av2[iv4] += stima*stima;
	err=Error(glob_av[iv4],glob_av2[iv4],iblk);
	OrderPar << iblk << "   ";
	OrderPar << setprecision(prec) << glob_av[iv4]/(long double)iblk << "   ";
    OrderPar <<setprecision(prec) << err << endl;
	...
	//g(a)
	for(k=igang; k<igang+nbins_angle; ++k) {

		kk=k-igang; 
		//a = kk*bin_size_angle + 0.75;
		gdia = mpi_blk_av[k]/mpi_blk_norm;
		glob_av[k] += gdia;
		glob_av2[k] += gdia*gdia;
		if(iblk == nblk)
		{
			err=Error(glob_av[k],glob_av2[k],iblk);
			Gangle << setprecision(prec) << ((double)kk + 0.5)*bin_size_angle + left_value_angle  << "   ";
			Gangle << setprecision(prec) << glob_av[k]/(double)iblk 		<< "   ";
			Gangle << err << "   " << endl;
		}
	...
	//linear distrib x
	for(k=ildcm_x; k<ildcm_x+nbins_ld_cm_x;++k) {

		kk = k-ildcm_x;
		ldcm_x = mpi_blk_av[k]/mpi_blk_norm;
		glob_av[k] += ldcm_x;
		glob_av2[k] += ldcm_x*ldcm_x;
		if(iblk == nblk)
		{
			err=Error(glob_av[k],glob_av2[k],iblk);
			LinDistX << setprecision(prec) << ((double)kk + 0.5)*bin_size_ld_cm_x + left_value_ld_x << "   ";
			LinDistX << setprecision(prec) << glob_av[k]/(double)iblk  << "   ";
			LinDistX << err << "   " << endl;
		} 
	...
	//linear distrib o1
	for(k=ildcm_o1; k<ildcm_o1+nbins_ld_cm_o1;++k) {

		kk = k-ildcm_o1;
		ldcm_o1 = mpi_blk_av[k]/mpi_blk_norm;
		glob_av[k] += ldcm_o1;
		glob_av2[k] += ldcm_o1*ldcm_o1;
		if(iblk == nblk)
		{
			err=Error(glob_av[k],glob_av2[k],iblk);
			LinDistO1 << setprecision(prec) << ((double)kk + 0.5)*bin_size_ld_cm_o1 + left_value_ld_o1 << "   ";
			LinDistO1 << setprecision(prec) << glob_av[k]/(double)iblk  << "   ";
			LinDistO1 << err << "   " << endl;
		} 
	...
	//linear distrib o2
	for(k=ildcm_o2; k<ildcm_o2+nbins_ld_cm_o2;++k) {

		kk = k-ildcm_o2;
		ldcm_o2 = mpi_blk_av[k]/mpi_blk_norm;
		glob_av[k] += ldcm_o2;
		glob_av2[k] += ldcm_o2*ldcm_o2;
		if(iblk == nblk)
		{
			err=Error(glob_av[k],glob_av2[k],iblk);
			LinDistO2 << setprecision(prec) << ((double)kk + 0.5)*bin_size_ld_cm_o2 + left_value_ld_o2 << "   ";
			LinDistO2 << setprecision(prec) << glob_av[k]/(double)iblk  << "   ";
			LinDistO2 << err << "   " << endl;
		} 
	...
	//energy distrib
	for(k=iene; k<iene+nbins_ene;++k) {

		kk = k-iene;
		ene_dist= mpi_blk_av[k]/mpi_blk_norm;
		glob_av[k] += ene_dist;
		glob_av2[k] += ene_dist*ene_dist;
		if(iblk==nblk)
		{
			err=Error(glob_av[k],glob_av2[k],iblk);
			EneDist << setprecision(prec) << ((double)kk + 0.5)*bin_size_ene + left_value_ene << "   ";
			EneDist << setprecision(prec) << glob_av[k]/(double)iblk  << "   ";
			EneDist << err << "   " << endl;
		}
	...
	//boxx distribution
	for(k=boxx; k<iboxx+nbins_box;++k) {

		kk = k-iboxx;
		boxx_dist = mpi_blk_av[k]/mpi_blk_norm;
		glob_av[k] += boxx_dist;
		glob_av2[k] += boxx_dist*boxx_dist;
		if(iblk==nblk)
		{
			err=Error(glob_av[k],glob_av2[k],iblk);
			BoxxDist << setprecision(prec) << ((double)kk + 0.5)*bin_size_boxx + left_value_boxx << "   ";
			BoxxDist << setprecision(prec) << glob_av[k]/(double)iblk  << "   ";
			BoxxDist << err << "   " << endl;
		}
	...
	//boxy distribution
	for(k=boxy; k<iboxx+nbins_box;++k) {

		kk = k-iboxy;
		boxy_dist = mpi_blk_av[k]/mpi_blk_norm;
		glob_av[k] += boxy_dist;
		glob_av2[k] += boxy_dist*boxy_dist;
		if(iblk==nblk)
		{
			err=Error(glob_av[k],glob_av2[k],iblk);
			BoxyDist << setprecision(prec) << ((double)kk + 0.5)*bin_size_boxy + left_value_boxy << "   ";
			BoxyDist << setprecision(prec) << glob_av[k]/(double)iblk  << "   ";
			BoxyDist << err << "   " << endl;
		}
...
{
	ofstream file_output1, file_output2;
	if (my_rank==rank_0) {
		
		ofstream WriteSeed, WriteRestart;
		...
		WriteRestart << u1x << endl;
		WriteRestart << u2x << endl;
		WriteRestart << u2y << endl;
		WriteRestart << nx << endl;
		WriteRestart << ny << endl;
		WriteRestart << ncluster << endl;
		WriteRestart << rho << endl;
   		WriteRestart << atan(tga) << endl;
		...
		cout << "Print final configuration of different cores in directory OutputConfig" << endl << endl;

	}

	file_output1.open("OutputConfig/output.config.final" + to_string(my_rank) + ".txt");				// Units divided by box side
	file_output2.open("OutputConfig/output.aconfig." + to_string(my_rank) +"txt");						// Print spin xm, ym, phi, dr
...
//print istant distribution of different cores
void Print_Cores(int iblk)
{
		ofstream output1("OutputSpecificHeatCores/output.specific_heat_"+ to_string(my_rank) +".txt", ios::app);
		ofstream output2("OutputEpotCores/output.epot_"+ to_string(my_rank) + ".txt",ios::app);
		ofstream output3;
		ofstream output4;
		ofstream output5;
		ofstream output6;
		if(iblk==nblk) {
			output3.open("OutputEpotDistribCores/output.energy.distribution_"+ to_string(my_rank) +".txt");
			output4.open("OutputAngleDistribCores/output.gangle_"+to_string(my_rank)+".txt");
			output5.open("OutputBoxxDistribCores/output.boxx.distribution_"+ to_string(my_rank)+".txt");
			output6.open("OutputBoxyDistribCores/output.boxy.distribution_"+to_string(my_rank)+".txt");
		}
 
		output1 << iblk << "   " << setprecision(prec) << (blk_av[iv2]/blk_norm-(blk_av[iv3]/blk_norm)*(blk_av[iv3])/blk_norm)/(long double)(temp*temp*npart) << endl;
		output2 << iblk << "   " << setprecision(prec) << walker[iv3]/(long double)npart <<  endl; 
		
		for(int k=iene; k<iene+nbins_ene;++k) {

			int kk = k - iene;
			ene_dist[kk] += blk_av[k]/blk_norm;
		
			if(iblk==nblk) {
				output3 << setprecision(prec) << ((double)kk + 0.5)*bin_size_ene + left_value_ene << "   ";
				output3 << setprecision(prec) << ene_dist[kk]/(double)iblk  << endl;
			}
		}

		for(int k=igang; k<igang+nbins_angle;++k) {

			int kk = k - igang;
			angle_dist[kk] += blk_av[k]/blk_norm;
		
			if(iblk==nblk) {
				output4 << setprecision(prec) << ((double)kk + 0.5)*bin_size_angle + left_value_angle << "   ";
				output4 << setprecision(prec) << angle_dist[kk]/(double)iblk  << endl;
			}
		}

		for(int k=iboxx; k<iboxx+nbins_box;++k) {

			int kk = k - iboxx;
			boxx_dist[kk] += blk_av[k]/blk_norm;
		
			if(iblk==nblk) {
				output5 << setprecision(prec) << ((double)kk + 0.5)*bin_size_boxx + left_value_boxx << "   ";
				output5 << setprecision(prec) << boxx_dist[kk]/(double)iblk  << endl;
			}
		}

		for(int k=iboxy; k<iboxy+nbins_box;++k) {

			int kk = k - iboxy;
			boxy_dist[kk] += blk_av[k]/blk_norm;
		
			if(iblk==nblk) {
				output6 << setprecision(prec) << ((double)kk + 0.5)*bin_size_boxy + left_value_boxy << "   ";
				output6 << setprecision(prec) << boxy_dist[kk]/(double)iblk  << endl;
			}
		}
